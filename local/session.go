...
func getSessionFromRedis(un string) (bool, string) {
	exist := false
	sess, getSessionErr := redis.String(rdb.Do("HGET", un, "session"))
	//fmt.Println(sess)
	checkErr(getSessionErr, "get session from redis err")
...
func verifyLoginStatus(r *http.Request) bool {
	...
	checkInfo(getCookieValueErr, "can not get username from cookie")
	if getCookieValueErr == nil {
		clientSess := newHashSession(cu.Value)
		serverSessExist, serverSess := getSessionFromRedis(cu.Value)
...
func verifyPasswordByUsername(username, password string) bool {
	truePassword := false
	serverHashPW, exist := getHashedPWByUsername(username)
	if exist == true {
